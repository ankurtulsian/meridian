...
## How we work — the protocol's own record

Rasoi's notebook and the working style's notebook were separate files on separate branches
until they were merged on 31 Aug. This section is what the second one held: the threads that
built the protocol, and the decisions taken about it. It is history, not live work — nothing
here is open — but it is the reasoning behind `.claude/working-style.md`, and deleting it in
a merge would have been exactly the failure that document warns about.

Its numbering (D1, D2 …) is deliberately kept separate from Rasoi's (1, 2 …). They count
different things: these are decisions about how the work is run, those are decisions about
what gets cooked.
